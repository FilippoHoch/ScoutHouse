...
def upgrade() -> None:
    ...
    # autogenerated name ``structures_slug_key``.  When this migration is run
    # against an already-upgraded database (for example after a failed deploy
    # that partially applied the migration), the constraint may already have
    # been removed.  We inspect the table first so that the migration can be
    # applied idempotently.
    inspector = sa.inspect(bind)
    existing_constraints = {
        constraint["name"] for constraint in inspector.get_unique_constraints("structures")
    }
    if "structures_slug_key" in existing_constraints:
